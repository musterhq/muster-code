# the reference IDE parity — the acceptance spec (owner, 2026-09-03)

Every line is an acceptance criterion. Status: ✅ done (verified live) · 🔧 in progress · ⬜ next · V2 = second wave.
Visual fidelity for each item is judged against `docs/cursor-ux-spec.md` and the owner's the reference IDE screenshots.

## Main IDE shell — mandatory
- ✅ Activity/sidebar icons (horizontal, the reference IDE placement) · ✅ File explorer · ✅ Tabs · ✅ Monaco editor · ✅ Breadcrumbs
...
- 🔧 Empty-editor shortcut list in the reference IDE's wording (New Agent ⇧⌘L, Show Terminal ⌘J, Search Files ⌘P, Open Browser ⇧⌘B, Maximize Chat ⌥⌘E, Add Repository ⌥⌘A)
...
## Mined from the reference IDE (the installed app's own bundle, not screenshots)

Sources: `docs/reference/cursor-inline-diff.css`, `docs/reference/cursor-widgets.css`, `docs/reference/cursor-tokens.txt` (extracted from `the reference IDE/…/workbench.desktop.main.{css,js}`).

### Inline diff (agent edits in the file's own editor)
- Not a diff editor. Added lines: whole-line background `diffEditor.insertedLineBackground` (reference-IDE-dark `#3FA26633`; fallback `rgba(12,233,27,.2)`), overview-ruler marks in `editorOverviewRuler.addedForeground`.
- Removed lines: view zones (`.inline-diff-removed`, no line numbers), syntax-tokenized, `opacity:.9`, `padding-left:.5px`, background `diffEditor.removedLineBackground` (reference-IDE-dark `#B8004933`).
...
- reference-IDE-dark: editor `#181818`, chrome `#141414`, fg `#F0F0F0`, `charts.green #3FA266`, `charts.red #E34671`, `button.background #81A1C1`, `button.foreground #191c22`, `input.background #F0F0F00A`, gutter added/deleted/modified `#3FA266 / #E34671 / #D2943E`.
...
- Realtime inline diff: DONE and verified live (the reference IDE architecture: workbench contribution + extension bridge). Original visible, red ghost rows (tokenized, no line numbers), green added lines streaming token by token, ruler marks, follow-scroll, per-hunk "n of m · Reject ⌘N · Accept ⌘Y" widgets (compact when text collides), bottom "Undo All / Keep All" bar, per-hunk/file/all accept-reject, clean disk convergence. Remaining polish listed in docs/HANDOFF.md §7a.
- Agent pane: composer + review bar exist; the the reference IDE chat rebuild (tabs, history, non-hardcoded models/modes incl. Kanban + access modes, inherited full history, diff preview cards, plan cards, approvals) is specified in docs/HANDOFF.md §7b — next.
...
- Next: verify plan mode end to end with a real turn; the reference IDE-exact plan editor toolbar (Preview ⌄ · model · Build) for `.plan.md`; diff preview block inside chat edit cards; @ context picker; Cmd-K; Tab.
...
- Modes now mirror the reference IDE's list with descriptions (Agent, Triage, Plan, Spec, Debug, Multitask, Ask, Project + custom); ⌘. / ⇧Tab open the mode menu; plan/spec → Codex plan mode; ask/project → read-only; triage → delegating effort; multitask → board.
...
- Density per the reference IDE: tool calls as 22px lines, tab row is the pane header (composite title hidden), compact bubbles/cards, welcome shortcuts in the empty editor (New Agent ⇧⌘L, Show Terminal ⌘J, Search Files ⌘P, Maximize Chat ⌥⌘E, Add Repository, Open Settings).
...
- Visual editor: the Selected section has editable text and style fields (color, background-color, font-size, font-weight, padding, margin, border-radius) applied live; the Changes tab lists them as old → new with revert, and "Apply changes in chat" hands them to the agent as a prompt (the reference IDE's CHANGES list).
...
**Pending**:
1. Terminal tool execution as the reference IDE cards: live stdout/stderr, Run / Skip, approval inside the chat (⏎ / ⇧⏎ / Esc) instead of VS Code notifications, background continuation.
2. Markdown renderer: file-path links, tables, copy/apply on code blocks, streaming caret, citations; Mermaid (V2).
3. the reference IDE Tab: partial accept (⌘→), next-edit prediction, snooze, status-bar state; the provider itself exists.
...
7. Settings pages reference-IDE-style for rules (enable state, auto/manual, Markdown editor), MCP (enable/disable, tools, health, auth, logs), skills, hooks, custom modes editor.
...
- **Full access = the reference IDE auto-apply**: the inline diff renders without per-hunk Accept/Reject widgets (`widgets:false`), the colours stay in the files, the bar offers Keep all / Undo all; Manual approval and Read only keep the review widgets.
...
## Tool cards, approvals in the chat, markdown — 2026-09-04
- **Approvals are chat cards** (the reference IDE's pending shell decision): `item/commandExecution/requestApproval`, `item/fileChange/requestApproval` and `mcpServer/elicitation/request` render a card (Run command / Apply changes / Permission request) with the command or files, cwd and reason, and the buttons Run ⏎ · Allow for session ⇧⏎ · Skip Esc (Allow/Decline for elicitations). ⏎ / ⇧⏎ / Esc answer the oldest pending card when the composer is empty or unfocused; a turn that ends settles anything still pending as declined. Verified through the harness `request` command (`{"cmd":"request","method":…,"params":…}` blocks until `{"cmd":"pane","decide":{id,decision}}`): decisions `accept` / `acceptForSession` / `decline` round-trip, elicitation maps to accept/decline.
- **Command cards**: terminal / globe / plug glyph, "Running" with an amber pulse, then "Ran", "Failed" (red, auto-expanded output) or "Skipped", `exit N` and duration from `item/completed`; output streams into the collapsible body.
- **Markdown**: file references are links that open the file at the line — backticked paths (`src/a.ts:12`), bare paths with a slash, and Codex citations `【F:path†L3-L4】` as chips; fenced blocks with a path in the info string (```ts src/x.ts) get a path header with Copy and **Apply** (hands the block to the agent for that file), others Copy and **Insert** (at the cursor of the active editor); task lists render check glyphs; a caret blinks at the end of the streaming answer. `scripts/dev/render-test.py` unit-tests the renderer outside the webview.
- Not yet: the reference IDE's "Move to background" for long commands; questions (`item/tool/requestUserInput`) still use quick picks.

## Context kinds — 2026-09-04
- New mention kinds, all expanded by `expandContext`: `@folder/` or `@folder:path` (tree, 3 levels, 200 entries, sizes); `@code:Name` (the symbol's source via workspace + document symbol providers, falling back to a `git grep` definition search when the language service has not loaded the project); `@link:url` or a bare URL (page text with HTML stripped, 10 s budget); `@rules` / `@rule:name` (`.muster/rules`, `.cursor/rules`); `@git:pr` (`gh pr view` + `gh pr diff`, else the branch diff). Tokens may now contain URL characters; trailing punctuation is trimmed.
...
- **Rules**: every rule shows the reference IDE's kind from its frontmatter — Always (`alwaysApply: true`, or a plain `.md` without frontmatter), Auto (`globs`), Agent (`description`), Manual — and an enable toggle (`muster.rules.disabled`, workspace setting). `readRules` now attaches them the way the reference IDE does: Always every turn; Auto when a mentioned file matches the globs; Agent as a one-line offer that `@rule:name` loads; Manual only when mentioned. "New rule" writes a frontmatter template. User rules (`~/.codex/AGENTS.md`) and the project AGENTS.md are listed.
...
## the reference IDE Tab, checkpoints, review editor, performance — 2026-09-04
- **Muster Tab**: partial accept ⌘→ (`editor.action.inlineSuggest.acceptNextWord`, Ctrl+→ elsewhere) while a suggestion shows; **next-edit prediction** — one JSON answer per pause carries the continuation at the cursor and the edit the recent change implies elsewhere, shown as an inline edit (Tab jumps and accepts); **snooze** (15 min / 1 h / until tomorrow / resume) and per-language disable from the status-bar item menu (`muster.completions.disabledLanguages`, the reference IDE's default markdown + plaintext), `muster.completions.nextEdit` toggle. Still opt-in (`muster.completions.enabled`).
- **Checkpoints**: Restore checkpoint reverts the files at once but defers the provider-side `thread/revert` until the next message, so **Redo checkpoint** brings the files and the messages back with the thread intact; Edit-and-resend reverts immediately.
- **Review Changes editor** (multi-diff): per-file **Keep File / Undo File** in each file header (`multiDiffEditor/resource/title`), **Keep all / Undo all** and the inline ↔ side-by-side toggle in the editor title while a live edit exists.
- **Performance (owner: settings and other things lagged even on a 32–48 GB Mac)**: every settings section, the mode/model pickers and the status chip spawned a throwaway app-server process (seconds each; the MCP section also made it connect to every MCP server). All catalog queries now go through a persisted stale-while-revalidate cache (`cachedQuery`, 10 min TTL, refreshed in the background, warmed one process at a time after activation). Measured: settings sections 0–1 ms after the first run; the pane's models/access come from the same cache. Settings shows "updated N min ago · Refresh".

## Shortcuts, dictation, history actions, browser extras — 2026-09-04
- **Shortcut parity pass** (from the atlas table, `when` clauses included): ⌘I / ⌘L open the chat (⌘L in an editor adds the selection), ⌘⇧L / ⌘⇧I new agent, ⌘T / ⌘N new tab and ⌘W close tab when the pane is focused, ⌘⇧⌫ stop, ⌘[ ⌘] and ⌘⌥← ⌘⌥→ previous/next chat tab, ⌘. / ⇧Tab mode menu, ⌘/ model menu, ⌘⌥P add context (@), ⌘⇧D "Investigate Error in Chat" (diagnostics under the cursor go to the agent with the file range), ⌘⇧R Review Changes, ⌘⌥U toggle the agent side bar, ⌘⇧V plan raw ↔ rich, ⌘⏎ Build in the plan editor, ⌘⇧⌫ reject file (was ⌘⌫), ⌘⇧Space dictation, ⌥⌘I browser DevTools, ⌘→ partial accept.
- **Dictation**: the mic button or ⌘⇧Space runs `muster.dictation.command` (default `hear`, Apple's on-device recognition via `brew install hear`; any transcriber that prints lines works) and streams its lines into the composer at the caret; the mic glows while listening; a missing command explains itself. Verified through the harness (`{"cmd":"dictate","command":"printf …"}`).
- **History**: hover a thread → Rename (`thread/name/set`), Export as Markdown (save dialog, `.muster/exports`), Archive (`thread/archive`), plus Pin.
- **Browser**: bookmarks bar under the address bar with a ☆ toggle (workspace state), DevTools button (detached), certificate overlay for untrusted hosts with "Proceed anyway" (host trusted for the session; localhost always trusted), headless tabs for the agent (`browser_navigate` with `headless: true` opens a hidden tab, `browser_tabs` marks them).
- Left: tools for Claude turns (`--mcp-config`), the reference IDE's "Move to background" for long commands, questions (`requestUserInput`) as cards, side preview in the @ menu, projects / MCP attachments, V2 (Agents window, background agents, worktrees, git review).
- Claude turns get the browser tools too: Claude Code runs with `--mcp-config` pointing at the same launcher (`mcp__muster_browser` allowed, the browser note appended to its system prompt). Not yet exercised with a live Claude turn.
