...
| Workflow surface | Codex / upstream evidence | Cursor | OpenCode | Zed | Muster status and decision |
|---|---|---|---|---|---|
...
Scheduling remains outside the Agent workspace until the host chooses a supported automation integration. Cursor worktrees, OpenAI App Server APIs, and child-session references establish the target shape; they do not claim current Muster runtime support.
