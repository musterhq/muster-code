...
- **Approvals are chat cards** (Cursor's pending shell decision): `item/commandExecution/requestApproval`, `item/fileChange/requestApproval` and `mcpServer/elicitation/request` render a card (Run command / Apply changes / Permission request) with the command or files, cwd and reason, and the buttons Run ⏎ · Allow for session ⇧⏎ · Skip Esc (Allow/Decline for elicitations). ⏎ / ⇧⏎ / Esc answer the oldest pending card when the composer is empty or unfocused; a turn that ends settles anything still pending as declined. Verified through the harness `request` command (`{"cmd":"request","method":…,"params":…}` blocks until `{"cmd":"pane","decide":{id,decision}}`): decisions `accept` / `acceptForSession` / `decline` round-trip, elicitation maps to accept/decline.
...
- Not yet: Cursor's "Move to background" for long commands; questions (`item/tool/requestUserInput`) still use quick picks.
...
## Cursor Tab, checkpoints, review editor, performance — 2026-09-04
- **Muster Tab**: partial accept ⌘→ (`editor.action.inlineSuggest.acceptNextWord`, Ctrl+→ elsewhere) while a suggestion shows; **next-edit prediction** — one JSON answer per pause carries the continuation at the cursor and the edit the recent change implies elsewhere, shown as an inline edit (Tab jumps and accepts); **snooze** (15 min / 1 h / until tomorrow / resume) and per-language disable from the status-bar item menu (`muster.completions.disabledLanguages`, Cursor's default markdown + plaintext), `muster.completions.nextEdit` toggle. Still opt-in (`muster.completions.enabled`).
...
- Left: tools for Claude turns (`--mcp-config`), Cursor's "Move to background" for long commands, questions (`requestUserInput`) as cards, side preview in the @ menu, projects / MCP attachments, V2 (Agents window, background agents, worktrees, git review).
