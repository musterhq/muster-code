...
> **Superseded implementation direction — read [the authoritative Terra handoff](TERRA-HANDOFF-2026-09-14.md) first.** It contains the current product intention, finish standard, features, interrupted agent state, crash blocker and exact continuation prompts. the reference IDE imitation, parent-only GUI work, and old usage-monitoring instructions below are historical and must not override the user's latest requirements.

> Current direction (2026-09-12): the owner now wants a distinct, customizable Muster UI. Exact the reference IDE imitation below is historical. Preserve working realtime/full-file diffs and use [the quality report](quality/2026-09-12/report.md) for the current changes, evidence, and release gates.

Read this first. It is written so that any builder (Codex included) can continue exactly the way the work has been done so far: research the reference IDE from its own bundle, build the the reference IDE shape, verify live in the running app, never guess.

## 0. Acceptance criteria and owner rulings (verbatim intent)

- **"CURSOR is the AIM AND ACCEPTANCE CRITERIA."** Replicate the reference IDE's UI/UX exactly. Not the VS Code / Copilot chat style. Verify against `/Applications/the reference IDE` (installed, logged in). "The nuances are very subtle yet effective" — mine the reference IDE's own CSS/JS for exact values (`docs/reference/`), do not eyeball.
- Standalone app, not an extension ("those work half assed"). Every VS Code feature + every the reference IDE feature + every Codex app feature (browser, annotations, computer use) "in a code environment to check what code is being changed".
...
- **Preview of the code diff inside the chat** (the reference IDE shows each edited file as a card with a +/- diff block in the message flow) is required.
...
├─ out/vs/workbench/workbench.desktop.main.css ← product/muster-workbench.css appended (the reference IDE skin + inline-diff widgets)
└─ extensions/muster.muster-code               ← packages/builtin (built by esbuild → dist-ext)
   extensions/muster.theme-muster              ← packages/theme (Muster Dark = reference-IDE-dark palette + layout defaults)
```
...
- **Why a workbench patch**: extension-API decorations cannot right-align widgets or be clicked, and webview insets fail under profile contention. the reference IDE itself is a workbench contribution (`acceptRejectPartialEditWidget`, `inline-diff-*`, `pure-ai-prompt-bar`); we mirror that.
...
| `product/muster-workbench.css` | the reference IDE skin for native chrome + `.muster-*` inline-diff widget styles (values from `docs/reference/`) |
...
| `docs/cursor-ux-spec.md` | Measured the reference IDE spec (tokens, composer, bubbles, cells) |
| `docs/cursor-parity-spec.md` | Owner's checklist with status + "Mined from the reference IDE" section |
| `docs/cursor-feature-atlas.md` | Feature-by-feature sweep of the reference IDE's bundle (extensions, decoded shortcuts, modes, plan mode, layouts, context kinds, settings) |
| `docs/reference/cursor-inline-diff.css`, `cursor-widgets.css`, `cursor-tokens.txt`, `*.png` | Raw extracts from the reference IDE's bundle + reference frames |
...
- Widgets now switch to a compact (shortcut-only) form when the line's text would collide (`textEnd + 32 > width − right − widgetWidth`), as the reference IDE does.

## 5. the reference IDE facts that drive the build (all mined; details in the reference files)
...
a. **Inline diff polish**: verify the three staged fixes; word-level inner-change boxes (diff within changed line pairs); first-line right fade; multi-file bar ("Keep all changes" + next file); `Add File`/`Delete File`/`Move to` visuals; keybinding `when` should also require the active editor to be a live file (context key per resource); undo/redo semantics after accept; per-hunk widgets clickable (done) and hover states; light theme values from the reference IDE Light (`docs/reference`).

b. **Agent pane rebuild** (the reference IDE chat, nothing hardcoded):
   ...
   - Inherit threads: list via `thread/list` (+ local rollout discovery for unnamed ones), open via `thread/read`/`thread/resume` and render the **entire** history: human bubbles, assistant markdown, reasoning ("Thinking"/"Thought" details), tool cards (Read file, terminal with output), file-change cards **with the +/- diff preview block in the message** (the reference IDE's cell: file header, stat, expandable diff), plan cards, approvals as they were answered. Scroll to end, unlimited scrollback.
   ...
   - Plan mode: `turn/plan/updated` + `item/plan/delta` → plan card with to-dos and "View Plan / Build"; plans saved as `.plan.md` under the workspace (the reference IDE: `.cursor/plans/<name>.plan.md`; use `.muster/plans/`), opened in a rendered plan editor with the toolbar (§5).
...
f. **Rules / MCP / Skills / Hooks views**: `.muster/rules`, `mcpServerStatus/list`, `skills/list`, `hooks/list`, `plugin/list` in settings-style pages like the reference IDE's.
...
1. Before building a surface, mine the reference IDE's bundle for it (strings → component → CSS rules → tokens); record the facts in `docs/`.
2. Build; typecheck + unit-test; hot-swap; reload via the socket; drive it via `replay`/`exec`; screenshot; compare with the the reference IDE frame; iterate. Claim only what the screenshot shows.
