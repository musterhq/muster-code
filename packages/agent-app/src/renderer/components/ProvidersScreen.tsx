...
export function ProvidersScreen() {
  ...
  const list=providers.value ?? [];const ready=list.filter(p=>p.available);const localProfiles=list.filter(p=>!p.custom&&!p.available&&p.status!=='not-detected');const custom=list.filter(p=>p.custom);const absent=list.filter(p=>!p.custom&&p.status==='not-detected');
