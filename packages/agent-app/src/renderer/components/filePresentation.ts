export type FilePresentation = 'markdown' | 'json' | 'csv' | 'tsv' | 'image' | 'text';
...
export function parseDelimited(text: string, separator: ',' | '\t'): {rows: string[][]; limited: boolean} {
  ...
  const cell = () => { row.push(value); value = ''; closed = false; if (row.length > 100) throw new Error('Preview supports up to 100 columns. Open source to inspect this file.'); };
  for (let i = 0; i < text.length; i++) {
    ...
    if (quoted) {
      ...
      if (rows.length >= 500 && i < text.length-1) return {rows, limited: true};
