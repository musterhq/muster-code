...
export function TerminalAgentAccess({chatId}:{chatId:string}):React.ReactElement {
  ...
  return <button type="button" className={`terminal-tool${allowed?' is-on':''}`} aria-pressed={allowed} aria-label={label} disabled={busy}
    title={allowed?'The agent can read this chat’s terminal output (read-only, secrets redacted). Click to revoke.':'Let the agent read this chat’s terminal output from its next turn (read-only, secrets redacted).'}
    onClick={()=>void toggle()}>{allowed?<Eye size={13}/>:<EyeOff size={13}/>}</button>;
}
