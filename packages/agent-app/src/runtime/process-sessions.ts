...
import {appendCommandOutput,MAX_COMMAND_OUTPUT} from './command-output-buffer.ts';
import {isActiveProcess,type ProcessEvent,type ProcessLease,type ProcessListSnapshot,type ProcessMetadata,type ProcessRef,type ProcessSnapshot,type ProcessStart,type ProcessSummarySnapshot} from '../shared/process-protocol.ts';
...
export type ProcessAuthorize=(input:{chatId:string;operation:'read'|'start'|'stop'})=>Promise<{cwd?:string;fullAccessAcknowledged?:boolean}>;
...
export class ProcessSessions {
  ...
  constructor(private filePath:string,private authorize:ProcessAuthorize,private emit:(event:ProcessEvent)=>void) {
  ...
  private prune():void {
    ...
    for(const session of inactive.slice(0,Math.max(0,inactive.length-MAX_PROCESS_HISTORY))){this.sessions.delete(session.processId);this.metadataStatuses.delete(session.processId);this.queueMetadata();}
  }
  ...
  private async launch(input:ProcessStart,fingerprint:string):Promise<ProcessSnapshot> {
    ...
    const session:ProcessSnapshot={chatId:input.chatId,processId:`process:${randomUUID()}`,generation:1,sequence:0,status:'starting',label:input.label?.trim()||'Command',purpose:input.purpose??'command',command:input.command,...(input.args?{args:[...input.args]}:{}),startedAt:now,updatedAt:now,output:'',truncated:false,exitCode:null};
    ...
    if(this.closing||this.cancelled.has(session.processId)){session.status='stopped';this.cancelled.delete(session.processId);this.publish(session);await this.save();return clone(session);}
    ...
    catch{session.status='failed';session.error='The command could not be started.';this.publish(session);await this.save();return clone(session);}
    ...
    const output=(delta:string)=>{if(handle.finished)return;Object.assign(session,appendCommandOutput(session,delta));this.dirty.add(session.processId);if(!this.flushTimer)this.flushTimer=setTimeout(()=>this.flushOutput(),80);};
  ...
  private finish(session:ProcessSnapshot,handle:Handle,status:ProcessSnapshot['status'],exitCode:number|null,signal?:string,error?:string):void {
    if(handle.finished)return;handle.finished=true;clearTimeout(handle.timer);this.handles.delete(session.processId);this.dirty.delete(session.processId);
  ...
  private stopOwned(session:ProcessSnapshot):void {
    ...
    if(!handle){this.cancelled.add(session.processId);session.status='stopping';this.publish(session);return;}
  ...
  dispose():Promise<void> {
    ...
    this.disposal=(async()=>{
      ...
      clearTimeout(this.flushTimer);this.flushTimer=undefined;this.dirty.clear();await this.save();this.disposed=true;
