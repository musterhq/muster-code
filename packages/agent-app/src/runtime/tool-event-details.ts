...
export function toolEventDetails(item: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const key of ['type','command','cwd','title','name','path','query','server','tool','namespace','prompt','model','senderThreadId']) {
  ...
  if (Array.isArray(item.commandActions)) result.commandActions = item.commandActions.slice(0,256).filter(isObject).map(action=>pick(action,['type','command','path','name','query']));
