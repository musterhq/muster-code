...
import {applyVisibility, costLabel, isModelHidden, estimateCostUsd, formatTokenCount, formatUsd, INCREMENTAL_INPUT_NOTE, modelBadges, modelKey, normalizeModelPolicy, summarizeUsage, validatePricing, ZERO_USAGE} from '../src/shared/model-catalog.ts';
...
test('PRO-04: the visibility policy persists, validates and resets', async t => {
  ...
  const shownAgain = await call(reread, 'models.policy.setHidden', {key, hidden: false});
  assert.deepEqual(shownAgain.hidden, []);
  assert.deepEqual(shownAgain.shown, [key], 'switching on is remembered, so a model that is off by default stays on');
  assert.equal(isModelHidden(shownAgain, 'hybrow', 'codex/gpt-5.6-luna', true), false);
  assert.equal(isModelHidden(shownAgain, 'hybrow', 'cursor/other', true), true, 'off by default until switched on');
  assert.equal(isModelHidden(shownAgain, 'hybrow', 'cursor/other'), false, 'a normal model is on unless switched off');
  await call(reread, 'models.policy.setPricing', {key, pricing: {inputPerMTok: 1, outputPerMTok: 2}});
  assert.deepEqual(await call(reread, 'models.policy.reset'), {hidden: [], shown: [], pricing: {}});
});
