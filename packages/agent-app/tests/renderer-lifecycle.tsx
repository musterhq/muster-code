...
window.muster={subscribe(listener:any){eventListener=listener;return()=>{}},async invoke(c:string,p:any){if(c==='files.read')return {path:p.path,text:p.path==='guide.md'?documentText:p.path==='large.md'?'x'.repeat(70000):p.path==='data.json'?JSON.stringify({nested:{answer:42}}):p.path==='invalid.json'?'{broken':p.path==='table.csv'?'Name,Note\nAlpha,Ready':'Version '+fileVersion,truncated:false};if(c==='workspace.watch')return; if(c==='app.snapshot')return {chats,folders:[],projects:[],version:1,activeChatId:'a'};if(c==='chat.select')return [{id:p.id+'1',chatId:p.id,kind:'assistant',text:'Chat '+p.id,createdAt:''}];}};
...
assert.match(document.querySelector('[aria-label="Delimited file preview"]')!.textContent!,/Alpha/);
store.closeTab(store.getState().activeTabId!);
