...
test('command search distinguishes local skills from unavailable plugin management',()=>{
  assert.deepEqual(filterComposerCommands('plugins').map(command=>command.id),['plugins']);
  assert.equal(COMPOSER_COMMANDS.find(command=>command.id==='plugins')?.description,'Unavailable in this build');
