...
const TOOLS = [
  { name: "browser_navigate", description: "Open a URL in the IDE's built-in browser tab (the one the user is looking at; preferred over any other browser automation) and wait for it to load. Returns a page snapshot. headless=true opens a hidden tab instead (for checks the user need not watch).", inputSchema: target({ url: { type: "string" }, headless: { type: "boolean" } }, ["url"]) },
