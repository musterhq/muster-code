...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  startDevControl(context, { live, log: (line) => output.appendLine(line), pane: { debugState: () => pane.debugState(), debugInput: (text) => pane.debugInput(text), debugSuggest: (kind, query, mode) => pane.debugSuggest(kind, query, mode), debugThreads: () => pane.debugThreads(), harness: (input) => pane.harness(input) } });
